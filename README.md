## Hi there 👋

I'm a passionate software engineer with expertise in full-stack development, systems programming, and artificial intelligence. My work spans across web applications, compilers, operating systems, and competitive programming.

## 🚀 What I Can Do

### 🌐 Full-Stack Web Development
- **Modern Web Applications**: Build responsive, scalable web apps using React, Next.js, and TypeScript
- **Backend Systems**: Develop robust REST APIs with Spring Boot, Node.js, and Flask
- **Database Design**: Design and implement databases with PostgreSQL, MongoDB, and MySQL
- **Cloud & DevOps**: Deploy applications using Docker, containerization, and CI/CD pipelines

**Featured Projects:**
- **[PCBanabo](https://github.com/Munzer2/PCBanabo)** - Full-stack PC configurator with AI-powered build recommendations, web scraping, and community features
- **[Jobathon](https://github.com/Munzer2/Jobathon)** - Job portal built with MERN stack (MongoDB, Express, React, Node.js)
- **[NextJS-dashboard](https://github.com/Munzer2/NextJS-dashboard)** - Modern dashboard with Next.js and TypeScript
- **[BookStore](https://github.com/Munzer2/BookStore)** - E-commerce platform using Spring Boot and MySQL

### 💻 Systems Programming & Low-Level Development
- **Compiler Construction**: Built a complete compiler with lexical analysis, parsing, and code generation
- **Operating Systems**: Implemented OS concepts including process scheduling, memory management, and file systems
- **Microcontroller Programming**: Assembly language programming for 8086 microcontrollers
- **Computer Graphics**: OpenGL-based graphics rendering and animations

**Academic Projects:**
- **[Compiler Sessional](https://github.com/Munzer2/Cse310-Compiler_Sessional)** - Complete compiler implementation in C
- **[OS Sessional](https://github.com/Munzer2/CSE-314-OS)** - Operating system concepts in C++
- **[Microcontrollers](https://github.com/Munzer2/CSE-316-Microcontrollers_Sessional)** - Assembly programming projects

### 🤖 Artificial Intelligence & Data Science
- **AI Algorithms**: Implementation of search algorithms, optimization techniques, and machine learning models
- **Data Analysis**: Statistical analysis and data visualization using Python and Jupyter Notebooks
- **Natural Language Processing**: Integration of AI models (Gemini API) for intelligent applications

**Projects:**
- **[AI Sessional](https://github.com/Munzer2/CSE-318-AI)** - AI algorithm implementations
- **[Datathon Project](https://github.com/Munzer2/CUET-Televerse-Datathon)** - Data analysis competition entry

### 🛡️ Security & Cryptography
- **Computer Security**: Implementation of security protocols and cryptographic algorithms
- **Secure Authentication**: JWT-based authentication and authorization systems

**Projects:**
- **[Security Projects](https://github.com/Munzer2/CSE-406-Computer-Security)** - Security implementations in Python
- **[Security Sessional](https://github.com/Munzer2/CSE-406_Security_Project)** - Security protocols and implementations

### 🏆 Competitive Programming
- **Problem Solving**: Extensive experience with algorithms and data structures
- **Code Optimization**: Writing efficient, optimized solutions in C++

**Repository:**
- **[CP Solutions](https://github.com/Munzer2/CP)** - Collection of competitive programming solutions

### 🎨 Software Engineering & Design
- **Design Patterns**: Implementation of classic software design patterns
- **Object-Oriented Programming**: Strong foundation in OOP principles and best practices
- **Software Architecture**: Building maintainable, scalable software systems

**Projects:**
- **[SWE Sessional](https://github.com/Munzer2/CSE-308_Software_Engineering_Sessional)** - Software engineering principles in Java

## 🛠️ Tech Stack

### Programming Languages
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![Assembly](https://img.shields.io/badge/Assembly-654FF0?style=flat&logo=assemblyscript&logoColor=white)

### Frontend Development
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat&logo=vite&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)

### Backend Development
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)

### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

### DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=flat&logo=apache-maven&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat&logo=selenium&logoColor=white)

### Other Technologies
- **Authentication**: JWT, OAuth
- **ORM**: JPA/Hibernate, Thymeleaf
- **Web Scraping**: Selenium WebDriver
- **Graphics**: OpenGL, iGraphics
- **Compilers**: Lex, Yacc
- **AI/ML**: Gemini API, Data Analysis
- **Testing**: Playwright

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Munzer2&show_icons=true&theme=radical)

## 📫 Get In Touch

Feel free to explore my repositories and reach out for collaboration opportunities!

---

⭐️ From [Munzer2](https://github.com/Munzer2)
